...
namespace strom {
    ...
    inline void Strom::run() {  ///begin_run
        ...
        try {
            ...
            _output_manager.reset(new OutputManager);   ///!ff
            ...
            if (num_free_parameters > 0) {
                _output_manager->outputConsole(boost::str(boost::format("\n%12s %12s %12s %12s") % "iteration" % "logLike" % "logPrior" % "TL"));   ///!g
                _output_manager->openTreeFile("trees.tre", _data);
                _output_manager->openParameterFile("params.txt", _model);   ///!gg
                chain.setTreeFromNewick(newick);
                chain.start();
                
                // // Output column headers and first line of output showing starting state (iteration 0) ///!h
                ...
                for (unsigned iteration = 1; iteration <= _num_iter; ++iteration) {
                    ...
                    sample(iteration, chain);   ///!j
                    
                    //if (iteration % _sample_freq == 0) { ///!k
                    //    GammaRateVarUpdater::SharedPtr ratevar_updater = std::dynamic_pointer_cast<GammaRateVarUpdater> (chain.findUpdaterByName("Gamma Rate Variance"));
                ...
                // Close output files   ///!l
                _output_manager->closeTreeFile();
                _output_manager->closeParameterFile(); ///!ll
                
                // Summarize updater efficiency ///!m
                ...
                _output_manager->outputConsole("MCMC skipped because there are no free parameters in the model");   ///!n
            }   
        }
        catch (XStrom & x) {
