...
namespace strom {
    ...
    inline void Likelihood::defineOperations(Tree::SharedPtr t) {   ///begin_defineOperations
        ...
        double relrate_normalizing_constant = _model->calcNormalizingConstantForSubsetRelRates(); ///!k
        ...
        for (auto & info : _instances) {
            ...
            unsigned instance_specific_subset_index = 0;
            for (unsigned s : info.subsets) {
                double subset_relative_rate = subset_relrates[s]/relrate_normalizing_constant;   ///!l
        ...
        if (info.invarmodel) {  ///!n
            ...
            for (unsigned s : info.subsets) {
                ...
                if (pinvar == 0.0) {
                    ...
                    auto interval = _data->getSubsetBeginEnd(s);
                    for (unsigned p = interval.first; p < interval.second; p++) {
                        // Loop through all states for this pattern             ///begin_states_loop
                        ...
                        double site_lnL = site_log_likelihoods[i++];            ///begin_sitelnL
