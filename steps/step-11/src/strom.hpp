...
namespace strom {
    ...
    class Strom {   ///begin_strom_class
        ...
        private:
            bool                                    processAssignmentString(const std::string & which, const std::string & definition);     ///!a
            ...
            bool                                    splitAssignmentString(const std::string & definition, std::vector<std::string> & vector_of_subset_names, std::vector<double>  & vector_of_values);  ///!b

            double                                  _expected_log_likelihood;   ///!c
    ...
    inline void Strom::processCommandLineOptions(int argc, const char * argv[]) {   ///begin_processCommandLineOptions
        std::vector<std::string> partition_statefreq;   ///!g
        ...
        std::vector<std::string> partition_ncateg;  ///!h
        std::vector<std::string> partition_subsets;
        std::vector<std::string> partition_relrates;    ///!ha
        std::vector<std::string> partition_tree;        ///!hb
        ...
        desc.add_options()
            ...
            ("ncateg,c", boost::program_options::value(&partition_ncateg), "number of categories in the discrete Gamma rate heterogeneity model")   ///!i
            ...
            ("expectedLnL", boost::program_options::value(&_expected_log_likelihood)->default_value(0.0), "log likelihood expected")    ///!j
        ...
        _model->setSubsetDataTypes(_partition->getSubsetDataTypes());     ///!k
        
        handleAssignmentStrings(vm, "statefreq", partition_statefreq, "default:equal"); ///!l
        ...
        handleAssignmentStrings(vm, "tree",      partition_tree,      "default:1"    ); ///!m
    }  ///end_processCommandLineOptions
    
    inline void Strom::handleAssignmentStrings(const boost::program_options::variables_map & vm, std::string label, const std::vector<std::string> & definitions, std::string default_definition) { ///begin_handleAssignmentStrings
        if (vm.count(label) > 0) {
            bool first = true;
    ...
    inline bool Strom::processAssignmentString(const std::string & which, const std::string & definition) { ///begin_processAssignmentString
    ...
    inline bool Strom::splitAssignmentString(const std::string & definition, std::vector<std::string> & vector_of_subset_names, std::vector<double>  & vector_of_values) {  ///begin_splitAssignmentString
        ...
        // Check for brackets indicating that parameter should be fixed     ///begin_bracket_check
    ...
    inline void Strom::run() {  ///begin_run
        ...
        try {
            ...
            _model->setSubsetNumPatterns(_data->calcNumPatternsVect()); ///!n
            ...
            std::cout << "\n*** Model description" << std::endl;    ///!p
            std::cout << _model->describeModel() << std::endl;
            _likelihood->setModel(_model);  ///!q
            ...
            if (_expected_log_likelihood != 0.0)    ///!r
                std::cout << boost::str(boost::format("      (expecting %.3f)") % _expected_log_likelihood) << std::endl;   ///!s
