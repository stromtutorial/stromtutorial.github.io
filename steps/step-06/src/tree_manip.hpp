...
#include <set>
#include <regex>
#include <boost/range/adaptor/reversed.hpp> ///!b
#include <boost/format.hpp>
#include "tree.hpp"
#include "xstrom.hpp" ///!c
...
namespace strom {
    ...
    class TreeManip {
        public:
            ...
            void                        buildFromNewick(const std::string newick, bool rooted, bool allow_polytomies);  ///!d
            void                        rerootAtNodeNumber(int node_number);    ///!e
        ...
        private:
            ...
            bool                        canHaveSibling(Node * nd, bool rooted, bool allow_polytomies);  ///!g
    ...
    inline void TreeManip::extractNodeNumberFromName(Node * nd, std::set<unsigned> & used) { ///begin_extractNodeNumberFromName
        assert(nd);
        bool success = true;
        unsigned x = 0;
        try {
    ...
    }   ///end_extractNodeNumberFromName

    inline void TreeManip::extractEdgeLen(Node * nd, std::string edge_length_string) {  ///begin_extractEdgeLen
        assert(nd);
        bool success = true;
        double d = 0.0;
        try {
    ...
    inline unsigned TreeManip::countNewickLeaves(const std::string newick) {    ///begin_countNewickLeaves
    ...
    }   ///end_countNewickLeaves

    inline void TreeManip::stripOutNexusComments(std::string & newick) {    ///begin_stripOutNexusComments
        std::regex commentexpr("\\[.*?\\]");
        newick = std::regex_replace(newick, commentexpr, std::string(""));
    }   ///end_stripOutNexusComments
    ...
    inline void TreeManip::refreshLevelorder() {    ///begin_refreshLevelorder
        if (!_tree->_root)
            return;
    ...
    inline bool TreeManip::canHaveSibling(Node * nd, bool rooted, bool allow_polytomies) {  ///begin_canHaveSibling
        assert(nd);
        if (!nd->_parent) {
    ...
    inline void TreeManip::rerootAtNodeNumber(int node_number) {    ///begin_rerootAtNodeNumber
    ...
    }   ///end_rerootAtNodeNumber

    inline void TreeManip::rerootAtNode(Node * prospective_root) {  ///begin_rerootAtNode
    ...
    inline void TreeManip::buildFromNewick(const std::string newick, bool rooted, bool allow_polytomies) {  ///begin_buildFromNewick
        ...
        try {
            ...
            for (auto ch : commentless_newick) {    ///begin_mainloop
                position_in_string++;
