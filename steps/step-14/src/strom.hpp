...
namespace strom {
    ...
    class Strom {
        ...
        private:
            ...
            Lot::SharedPtr                          _lot;           ///!c

            unsigned                                _random_seed;   ///!d
            unsigned                                _num_iter;      
            unsigned                                _print_freq;
            unsigned                                _sample_freq;   ///!e
    ...
    inline void Strom::processCommandLineOptions(int argc, const char * argv[]) {   ///begin_processCommandLineOptions
        ...
        desc.add_options()
            ...
            ("seed,z",        boost::program_options::value(&_random_seed)->default_value(1),   "pseudorandom number seed") ///!h
            ...
            ("samplefreq",  boost::program_options::value(&_sample_freq)->default_value(1),   "skip this many iterations before sampling next")  ///!i
    ...
    inline void Strom::run() {  ///begin_run
        ...
        try {
            ...
            std::string newick = _tree_summary->getNewick(0);   ///!j
            ...
            // Create a Lot object that generates (pseudo)random numbers    ///!k
            _lot = Lot::SharedPtr(new Lot);
            _lot->setSeed(_random_seed);    ///!kk
            
            // Create a Chain object and take _num_iter steps   ///!l
            Chain chain;
            ...
            if (num_free_parameters > 0) {
                ...
                // Output column headers and first line of output showing starting state (iteration 0)  ///!m
                ...
                std::cout << boost::str(boost::format("%12d %12.5f %12.5f %12.5f %12.1f %12d\n")
                    % 0
                    % chain.getLogLikelihood()
                    % chain.calcLogJointPrior()
                    % ratevar_updater->getCurrentPoint()
                    % 0
                    % 0);  ///!mm

                for (unsigned iteration = 1; iteration <= _num_iter; ++iteration) {  ///!n
                    chain.nextStep(iteration);  ///!nn
                    
                    if (iteration % _sample_freq == 0) {  ///!o
                        GammaRateVarUpdater::SharedPtr ratevar_updater = std::dynamic_pointer_cast<GammaRateVarUpdater> (chain.findUpdaterByName("Gamma Rate Variance"));
                        ...
                        if (log_prior == Updater::getLogZero())
                            std::cout << boost::str(boost::format("%12d %12.5f %12s %12.5f %12.1f %12d\n")
                                ...
                                % "-infinity"
                                % ratevar_updater->getCurrentPoint()
                                % ratevar_updater->getAcceptPct()
                                % ratevar_updater->getNumUpdates());
                        else
                            std::cout << boost::str(boost::format("%12d %12.5f %12.5f %12.5f %12.1f %12d\n")
                                % iteration
                                % chain.getLogLikelihood()
